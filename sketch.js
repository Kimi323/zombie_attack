...
function draw() {
    ...
    // drawheadshot should be in draw function otherwise the red circle will not appear on screen
    this.horde.drawHeadshot(mouseX, mouseY);
}

function mousePressed() {
    // reduce heath only when head is clicked
    this.horde.reduceHealth(mouseX, mouseY);
    this.horde.deleteZombie();
    // add zombie when one is deleted
    if (this.horde.zombies.length < 7) {
        this.horde.addZombies(1);
    } 
}
